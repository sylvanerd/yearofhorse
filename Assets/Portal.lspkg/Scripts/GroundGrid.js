...
function onFrameUpdated(eventData) {
    ...
    var curColor = groundMeshVisual.mainMaterial.mainPass.baseColor;

    // Determines if the user is touching/moving the object around to lerp the alpha for the grid up and down
    if (isTouching) {
        groundMeshVisual.mainMaterial.mainPass.baseColor = vec3.lerp(curColor, new vec3(1.0, 1.0, 1.0), script.fadeUpSpeed);
    } else {
        groundMeshVisual.mainMaterial.mainPass.baseColor = vec3.lerp(curColor, new vec3(0.0, 0.0, 0.0), script.fadeDownSpeed);
    }
